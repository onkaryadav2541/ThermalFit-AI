...
def draw_dashboard(frame, sensor_data, worst_leak, worst_score, is_locked, live_fit_score):
    h, w, _ = frame.shape
    cv2.rectangle(frame, (0, 0), (320, h), (15, 15, 15), -1)

    # Header
    cv2.putText(frame, "REAL-TIME ANALYSIS", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
    ...
    # Color logic: High score = Green, Low score = Red
    fit_color = (0, 255, 0) if live_fit_score > 60 else (0, 0, 255)

    cv2.putText(frame, "LIVE FIT FACTOR:", (10, 95), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
    cv2.putText(frame, f"{live_fit_score}/100", (180, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.8, fit_color, 2)

    # Fit Score Bar
    cv2.rectangle(frame, (10, 110), (10 + int(live_fit_score * 3), 120), fit_color, -1)
    cv2.rectangle(frame, (10, 110), (310, 120), (50, 50, 50), 1)
    # -----------------------------
    ...
    for i, (name, data) in enumerate(sensor_data.items()):
        ...
        y_pos = 170 + (i * 60)  # Shifted down to make room for Fit Score
...
def main():
    ...
    while True:
        ...
        # --- CALCULATE LIVE FIT SCORE ---
        # Fit Score = 100 - (Average Leak). So 0 leak = 100 fit.
        current_fit_score = 0
        if len(BASE_OFFSETS) > 0:
            avg_leak = total_leak_score / len(BASE_OFFSETS)
            current_fit_score = max(0, 100 - int(avg_leak))

        draw_dashboard(frame, sensor_data, worst_name, worst_score, is_locked, current_fit_score)
        cv2.imshow('AI Live Fit Analysis', frame)
